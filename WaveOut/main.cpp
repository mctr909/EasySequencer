...
/******************************************************************************/
int* WINAPI
waveout_GetActiveSamplersPtr() {
    return &gActiveCount;
}

CHANNEL** WINAPI
waveout_GetChannelPtr() {
    return gppChParams;
}

SAMPLER** WINAPI
waveout_GetSamplerPtr() {
    return gppSamplers;
}

/******************************************************************************/
LPBYTE WINAPI
waveout_LoadWaveTable(LPWSTR filePath, uint *size) {
    if (NULL == size) {
        return NULL;
    }
    //
...
waveout_Open() {
    ...
    CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)writeWaveOutBuffer, NULL, 0, &gThreadId);
    return true;
}
...
DWORD WINAPI writeWaveOutBuffer(LPVOID *param) {
    while (true) {
        ...
        //
        LPBYTE outBuff = (LPBYTE)gppWaveHdr[gWriteIndex]->lpData;
