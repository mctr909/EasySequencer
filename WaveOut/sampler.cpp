...
CHANNEL_VALUE** WINAPI
createChannels(SYSTEM_VALUE *pSys) {
...
void WINAPI
disposeSamplers(SAMPLER** ppSmpl, uint count) {
    if (NULL == ppSmpl) {
        return;
    }
    for (uint i = 0; i < count; ++i) {
...
void WINAPI
disposeChannels(CHANNEL_VALUE **ppCh) {
    if (NULL == ppCh) {
        return;
    }
...
inline void WINAPI
sampler(CHANNEL_VALUE **ppCh, SAMPLER *pSmpl, byte *pWaveBuffer) {
...
inline void WINAPI
channel32(CHANNEL_VALUE *pCh, float *outBuff) {
    ...
    for (; inputBuff < inputBuffTerm; inputBuff++, outBuff += 2) {
        // filter
        filter::lpf(&pCh->filter, *inputBuff * pCh->amp);
        // pan
...
inline void WINAPI
channel24(CHANNEL_VALUE *pCh, int24 *outBuff) {
    double *inputBuff = pCh->pWave;
    double *inputBuffTerm = inputBuff + pCh->pSystemValue->bufferLength;
    for (; inputBuff < inputBuffTerm; inputBuff++, outBuff += 2) {
        // filter
...
inline void WINAPI
channel16(CHANNEL_VALUE *pCh, short *outBuff) {
    double *inputBuff = pCh->pWave;
    double *inputBuffTerm = inputBuff + pCh->pSystemValue->bufferLength;
    for (; inputBuff < inputBuffTerm; inputBuff++, outBuff += 2) {
        // filter
